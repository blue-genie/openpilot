...
class CarState(CarStateBase):
  ...
  def _update_traffic_signals(self, CP, cp, cp_cam):
    if CP.carFingerprint in CANFD_CAR:
      ...
      vLimit = origVLimit if origVLimit not in (0, 254) else 0

      # speed in m/s
      self._vLimit = vLimit * speed_factor

      cloudlog.debug(f"origVLimit {origVLimit}, vLimit {vLimit},  origVLimitUnit {origVLimitUnit}, speed_factor {speed_factor}, _vLimit {self._vLimit}")

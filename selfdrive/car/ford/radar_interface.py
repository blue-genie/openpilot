...
class RadarInterface(RadarInterfaceBase):
  def __init__(self, CP):
    ...
    # print(f"Feed\tMRR Detection ID\tSubsegment ID\tScan Index\tLeft/Right(m)\tAgead(m)\tvRel\tTrack ID\tNew Signal")
  ...
  def _update_delphi_mrr_64(self):
    ...
    for ii in range(1, self.msg_count + 1):
      ...
      # if f"MRR_Detection_{ii:03d}" not in self.rcp.vl:
      #   continue
      ...
      for iii in range(1, 7):
        ...
        if valid:# and -10 < amplitude:
          ...
          # new_sig = 0
          # if f"CAN_DET_NEW_SIGNAL_{ii:02d}_{iii:02d}" in msg:
          #   new_sig = msg[f"CAN_DET_NEW_SIGNAL_{ii:02d}_{iii:02d}"]          # m/s [-128|127.984]
          dRel = cos(azimuth) * dist                             # m from front of car
          yRel = sin(azimuth) * dist                             # in car frame's y axis, left is positive
          
          if abs(yRel) > 10:
            # drop targets further than 10m left orright in from me
            dropped[0] +=1
            del self.pts[i]
            continue
          ...
          self.pts[i].measured = True
          # print(f"feed_{i}\t{ii}\t{iii}\t{scanIndex}\t{yRel}\t{dRel}\t{distRate}\t{self.pts[i].trackId }\t{amplitude}")

        else:
          del self.pts[i]

    # print(f"-\t-\t-\t-\t-\t-\t-\t-\t-")
    # pts_len = len(self.pts)
    # print(f"{pts_len}\t{dropped}")